...
from weather_sources import snapshots as weather
...
def test_compose_observed_weather_uses_forecast_fallback_when_goes_missing() -> None:
    ...
    assert payload["temp_250"] == pytest.approx(-42.0)
    assert payload["humidity_250"] == pytest.approx(56.0)
    assert payload["cloud_cover_high"] == pytest.approx(0.75)
    assert payload["precipitation"] == pytest.approx(1.0)
    assert payload["surface_visibility"] == pytest.approx(1.5)
    assert payload["fog_presence"] == pytest.approx(0.7)
    assert payload["cloud_optical_thickness"] == pytest.approx(0.61)
    assert payload["cloud_cover_grid"] == [0.2, 0.4, 0.7]
    assert payload["condensate_proxy"] == pytest.approx(0.58)
    assert payload["ice_cloud_fraction"] == pytest.approx(0.9)
    assert payload["ice_300mb"] == pytest.approx(0.2)
    assert payload["ice_250mb"] == pytest.approx(0.6)
    assert payload["ice_200mb"] == pytest.approx(0.7)
    assert payload["wind_shear_250"] == pytest.approx(0.15)
    assert payload["vertical_velocity_variance"] == pytest.approx(0.2)
    assert payload["metar_station"] == "KDAL"
    assert payload["metar_raw"] == {}
    assert math.isnan(payload["goes_cloud_cover"])
    assert payload["goes_cloud_cover_kind"] == ""
